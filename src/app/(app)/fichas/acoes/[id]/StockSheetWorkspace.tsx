...
export function StockSheetWorkspace({
  ...
  function handleFetch() {
    startTransition(async () => {
      const result = await fetchStockIndicatorsAction(ticker);
      if (!result.ok) {
      ...
      setMessage({
        tone: "success",
        text: `${suggestions.length} de 22 critérios preenchidos automaticamente a partir do investidor10.com.br. Revise antes de salvar.`,
      });
    });
  }
      ...
      <Card className="flex flex-wrap items-center justify-between gap-3 p-4">
        <div>
          ...
          <p className="text-xs text-ink-muted">
            Preenche automaticamente indicadores como ROE, Dividend Yield, Payout, Tag Along e outros — direto do
            investidor10.com.br, sem custo.
          </p>
        </div>
