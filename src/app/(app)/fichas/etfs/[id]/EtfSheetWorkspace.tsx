...
import { fetchEtfIndicatorsAction, fetchEtfOverviewAction } from "./actions";

type CriterionWithKey = FormCriterion & { key: string };

export function EtfSheetWorkspace({
  sheetId,
  basePath,
  ticker,
  criteria,
  ...
  useEffect(() => {
    let cancelled = false;
    startAnalysis(async () => {
      const r = await fetchEtfOverviewAction(ticker);
      if (cancelled) return;
      if (!r.ok) {
  ...
  function handleFetch() {
    startTransition(async () => {
      const result = await fetchEtfIndicatorsAction(ticker);
      if (!result.ok) {
