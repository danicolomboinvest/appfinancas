...
import { fetchFiiIndicatorsAction } from "./actions";

type CriterionWithKey = FormCriterion & { key: string };

export function FiiSheetWorkspace({
  sheetId,
  basePath,
  ticker,
  criteria,
  ...
  function handleFetch() {
    startTransition(async () => {
      const result = await fetchFiiIndicatorsAction(ticker);
      if (!result.ok) {
      ...
      <Card className="flex flex-wrap items-center justify-between gap-3 p-4">
        <div>
          ...
          <p className="text-xs text-ink-muted">
            Preenche automaticamente indicadores como Mandato, Segmento, Taxa de Administração, Vacância e outros —
            direto do investidor10.com.br, sem custo.
          </p>
        </div>
