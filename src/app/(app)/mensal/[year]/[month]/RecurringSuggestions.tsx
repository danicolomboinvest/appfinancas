...
export function RecurringSuggestions({ candidates, year, month }: { candidates: RecurringCandidate[]; year: number; month: number }) {
  ...
  function launch(c: RecurringCandidate, repeat: boolean) {
    startTransition(async () => {
      ...
      hide(c.key);
      showToast(repeat ? "Lançado neste mês e nos seguintes até dezembro." : "Lançado.");
    });
  }
  ...
  return (
    <Card className="flex flex-col gap-3 p-4">
      <div className="flex items-center gap-2">
        ...
        <p className="text-[15px] font-semibold text-ink">Parece que se repete</p>
      </div>
      <p className="text-caption text-ink-muted">Estava nos meses anteriores e ainda não está neste. Um toque lança.</p>
      <ul className="flex flex-col divide-y divide-border">
        {visible.map((c) => (
          <li key={c.key} className="flex flex-col gap-2 py-2.5 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <p className="truncate text-sm font-medium text-ink">{c.description || c.subcategory || "Lançamento"}</p>
              <p className="text-caption text-ink-faint">
                {money(c.amount, { round: true })}
                {c.typicalDay ? ` · costuma cair dia ${c.typicalDay}` : ""}
                {` · ${c.seenInMonths} dos últimos 3 meses`}
              </p>
            </div>
