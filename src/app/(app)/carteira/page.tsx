    ...
    <div className="flex flex-col gap-8">
      ...
      {(() => {
        ...
        const currentAllocationData = assets
          .filter((asset) => Number(asset.currentValue) > 0)
          .map((asset) => ({ name: asset.name, value: totalValue > 0 ? Number(asset.currentValue) / totalValue : 0 }));
        const idealAllocationData = assets
          .filter((asset) => asset.idealAllocationPercent !== null)
          .map((asset) => ({ name: asset.name, value: Number(asset.idealAllocationPercent) }));
