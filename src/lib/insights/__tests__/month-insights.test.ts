...
describe("biggestMoverInsight", () => {
  ...
  it("ignora categoria sem mês anterior (evita 'subiu infinito%')", () => {
    expect(biggestMoverInsight([category({ label: "Novo", amount: 500, previousAmount: null })])).toBeNull();
  });

  it("ignora valores pequenos demais pra virar alerta", () => {
    expect(biggestMoverInsight([category({ label: "Cafezinho", amount: 30, previousAmount: 10 })])).toBeNull();
  });

  it("queda também vira insight, com tom positivo", () => {
    const insight = biggestMoverInsight([category({ label: "Alimentação", amount: 600, previousAmount: 1000 })]);
    expect(insight?.tone).toBe("positive");
...
describe("buildMonthInsights", () => {
  ...
  it("mês sem histórico e sem categorias não inventa insight", () => {
    expect(buildMonthInsights({ currentExpense: 0, previousExpense: 0, categories: [] })).toEqual([]);
  });
});
